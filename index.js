...
async function run() {
  try {
    ...
    const intPackageCollection = client.db("worldholidaysDB").collection("International_packages");
    const domPackageCollection = client.db("worldholidaysDB").collection("DomesticPackage");
    const NextTour = client.db("worldholidaysDB").collection('NextTour');
    const Popular_Destination = client.db("worldholidaysDB").collection('PopularDestination');
    const DomesticeTickets = client.db('worldholidaysDB').collection('DomesticTicket');
    const InternationalTicket = client.db("worldholidaysDB").collection('Int_Ticket');
   const interNationalCartCollection = client.db("worldholidaysDB").collection('interNationalPackCart');
   const domCartCollection = client.db("worldholidaysDB").collection('domPackCart');
   const nextTourCartCollection = client.db("worldholidaysDB").collection('nextTourkCart');
   const popularDesCartCollection = client.db("worldholidaysDB").collection('popularDesCart');
 const ticketCartCollection = client.db("worldholidaysDB").collection('ticketCart');
   const otherServicesCartCollection = client.db("worldholidaysDB").collection('otherServicesCart');
   const visaProcessingFeeCollection = client.db("worldholidaysDB").collection('VisaProcessingFee');
   const visaProcessingFeeCartCollection = client.db("worldholidaysDB").collection('VisaProcessingFeeCart');
   const ourTeamMembers = client.db("worldholidaysDB").collection('OurTeam');
   const ourTravelersReview = client.db("worldholidaysDB").collection('travelersReview');
...
// }
  ...
  app.get("/users/admin/:email", verifyToken,  async (req, res) => { // Remove verifyAdmin
  ...
  app.get("/users/moderator/:email", verifyToken,  async (req, res) => { // Remove verifyAdmin
    const email = req.params.email;
    if (email !== req.decoded.email) {
      return res.status(403).send({ message: "forbidden access" });
    }
...
// all international api
    ...
    app.get("/show-all-international-packages", verifyToken, verifyAdminOrModerator, async (req, res) => {
      const result = await intPackageCollection.find().toArray();
      res.send(result);
    });
          ...
          const updatedDoc = {
              $set: {
                  ...
                  included_services: Array.isArray(data.included_services)
                      ...
                      : (data.included_services || '').split(',').map(item => item.trim()).filter(Boolean),
                  ...
                  tour_location_images: Array.isArray(data.tour_location_images)
                      ...
                      : (data.tour_location_images || '').split(',').map(item => item.trim()).filter(Boolean)
              }
          };
    ...
    app.delete('/show-all-international-packages/:id', verifyToken, verifyAdminOrModerator, async (req, res)=>{
      const id = req.params.id;
      const query = {_id : new ObjectId(id)};
    ...
    app.get("/domestic_packages", async (req, res) => {
      const result = await domPackageCollection.find().toArray();
      res.send(result);
    });
    app.get("/show-all-domestic-packages", verifyToken, verifyAdminOrModerator, async (req, res) => {
      const result = await domPackageCollection.find().toArray();
      res.send(result);
    });
    ...
    app.get('/show-all-domestic-packages/:id',   async (req, res)=>{
      const id = req.params.id;
      const query = {_id : new ObjectId(id)};
      const result = await domPackageCollection.findOne(query);
      res.send(result);

    } );
    app.patch('/show-all-domestic-packages/:id', async (req, res) => {
      try {
          const data = req.body;
          const id = req.params.id;
          ...
          const result = await domPackageCollection.updateOne(filter, updatedDoc);
    ...
    app.delete('/show-all-domestic-packages/:id', verifyToken, verifyAdminOrModerator, async (req, res)=>{
      const id = req.params.id;
      const query = {_id : new ObjectId(id)};
      const result = await domPackageCollection.deleteOne(query);
      res.send(result);

    } );

    app.post("/domestic_packages", verifyToken, verifyAdminOrModerator,  async(req, res)=>{
      const item = req.body;
      console.log(item)
      const result = await domPackageCollection.insertOne(item);
      res.send(result)
    })

    app.get("/domestic_packages/:id", async (req, res) => {
      const id = req.params.id;
      console.log(id);
      const query = { _id: new ObjectId(id) };
      const result = await domPackageCollection.findOne(query);
    ...
    app.get("/show-all-next_tour", verifyToken, verifyAdminOrModerator, async (req, res) => {
      const result = await NextTour.find().toArray();
      res.send(result);
    });


    app.delete('/show-all-next_tour/:id', verifyToken, verifyAdminOrModerator, async (req, res)=>{
      const id = req.params.id;
      const query = {_id : new ObjectId(id)};
    ...
    app.post("/next_tour", verifyToken, verifyAdminOrModerator,  async(req, res)=>{
      const item = req.body;
      console.log(item)
    ...
    app.patch('/show-all-next_tour/:id', async (req, res) => {
      try {
          ...
          if (result.modifiedCount === 0) {
              return res.status(404).send({ message: "Data not found or no changes made" });
          }
    ...
    app.post("/popular_destination", verifyToken, verifyAdminOrModerator,  async(req, res)=>{
      const item = req.body;
      console.log(item)
   ...
   app.get("/show-popular_destination", verifyToken, verifyAdminOrModerator, async (req, res) => {
    const result = await Popular_Destination.find().toArray();
    res.send(result);
  });

   app.delete('/show-popular_destination/:id', verifyToken, verifyAdminOrModerator, async (req, res)=>{
    const id = req.params.id;
    const query = {_id : new ObjectId(id)};
  ...
  app.get("/int_packages_cart_orders",verifyToken , verifyAdminOrModerator, async(req, res)=>{
  ...
  app.get("/dom_packages_cart_orders",verifyToken , verifyAdminOrModerator, async(req, res)=>{
  ...
  app.get("/next_tour_carts_orders",verifyToken , verifyAdminOrModerator, async(req, res)=>{
  ...
  app.get("/popular_des_carts_orders",verifyToken , verifyAdminOrModerator, async(req, res)=>{
  ...
  app.get("/ticket_carts_orders",verifyToken , verifyAdminOrModerator, async(req, res)=>{
  ...
  app.get("/visa-processing-fee-carts_orders",verifyToken , verifyAdminOrModerator, async(req, res)=>{
  ...
  app.get('/visa_cart', async(req, res)=>{
    const email = req.query.email;
    const query = {email:email}
    const result = await visaProcessingFeeCartCollection.find(query).toArray();
    res.send(result)
  });
  ...
  app.post("/visa-processing-fee", verifyToken, verifyAdminOrModerator,  async(req, res)=>{
    const item = req.body;
    console.log(item)
  ...
  app.get("/show-visa-processing-fee", verifyToken, verifyAdminOrModerator, async (req, res) => {
    const result = await visaProcessingFeeCollection.find().toArray();
    res.send(result);
  });


  app.delete('/show-visa-processing-fee/:id', verifyToken, verifyAdminOrModerator, async (req, res)=>{
    const id = req.params.id;
    const query = {_id : new ObjectId(id)};
  ...
  app.patch('/show-visa-processing-fee/:id', async (req, res) => {
    try {
        ...
        const result = await visaProcessingFeeCollection.updateOne(filter, updatedDoc);
...
app.get("/ourTeamMembers", async(req, res)=>{
 ...
 app.post('/ourTeamMembers', verifyToken, verifyAdminOrModerator, async(req, res)=>{
  const cartItems = req.body;
  const result = await ourTeamMembers.insertOne(cartItems);
  res.send(result);
});

app.get("/show-ourTeamMembers", verifyToken, verifyAdminOrModerator, async (req, res) => {
  const result = await ourTeamMembers.find().toArray();
  res.send(result);
});


app.delete('/show-ourTeamMembers/:id', verifyToken, verifyAdminOrModerator, async (req, res)=>{
  const id = req.params.id;
  const query = {_id : new ObjectId(id)};
...
app.get("/travelersReview", async(req, res)=>{
  const result = await ourTravelersReview.find({ status: 'approved' }).toArray();
  res.send(result)
 });

app.get("/show-travelersReview", verifyToken, verifyAdminOrModerator, async(req, res)=>{
